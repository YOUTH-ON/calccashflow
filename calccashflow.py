...
if 'pl_data' not in st.session_state:
    ...
    for cat in CATS: st.session_state[f"edit_mode_{cat}"] = False
...
if app_mode == "通常モード (受注案件)":
    st.title("💰 通常シミュレーション")
    st.session_state.normal_df = st.data_editor(st.session_state.normal_df, num_rows="dynamic", use_container_width=True, hide_index=True, key="ed_normal")
    ...
    for cat in CATS:
        with st.expander(f"📌 {cat}", expanded=True):
            ...
            if not st.session_state[f"edit_mode_{cat}"]:
                if col_r.button("編集", key=f"e_{cat}"):
                    st.session_state[f"edit_mode_{cat}"] = True; st.rerun()
                st.dataframe(st.session_state.pl_data[cat], use_container_width=True, hide_index=True)
            else:
                if col_r.button("保存", key=f"s_{cat}"):
                    st.session_state[f"edit_mode_{cat}"] = False; st.rerun()
                st.session_state.pl_data[cat] = st.data_editor(st.session_state.pl_data[cat], num_rows="dynamic", use_container_width=True, hide_index=True, key=f"ed_stable_{cat}")
...
if st.session_state.calc_done:
    ...
    ms = st.data_editor(st.session_state.manual_summary, use_container_width=True, hide_index=False, key="final_ed")
    ...
    def generate_pdf():
        ...
        elements.append(Paragraph("財務シミュレーション報告書", t_style))
    ...
    csv = final_view.to_csv().encode('utf_8_sig')
    st.sidebar.download_button("📊 CSVダウンロード", data=csv, file_name="data.csv", mime="text/csv")
